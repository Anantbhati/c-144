...
while (cap.isOpened()):
    ...
    mask_1 =cv2.morphologyEx(mask_1,cv2.MORPH_OPEN,np.ones((3,3),dtype='uint8'))
    mask_1 =cv2.morphologyEx(mask_1,cv2.MORPH_DILATE,np.ones((3,3),dtype='uint8'))

    #Selecting only the part that does not have mask one and saving in mask 2
    mask_2 =cv2.bitwise_not(mask_1,cv2.MORPH_OPEN,np.ones((3,3),np.uint8))
